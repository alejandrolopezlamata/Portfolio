...
export const Model = ({
  ...
  useEffect(() => {
    ...
    // Luces algo más fuertes para que no quede tan oscuro
...
const Device = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { texture, position, url, scale = 1 } = model;
      const hasTexture =
        texture && texture.placeholder && texture.placeholder.src;
      ...
      gltf.scene.traverse(async node => {
        // Oscurece cuerpo de dispositivos pero NO la pantalla
        if (node.material && node.name !== MeshType.Screen) {
          node.material.color = new Color(0x1f2025);
          node.material.color.convertSRGBToLinear();
        }

        if (hasTexture && node.name === MeshType.Screen) {
          // Aplica directamente la textura final (sin clon ni fade)
